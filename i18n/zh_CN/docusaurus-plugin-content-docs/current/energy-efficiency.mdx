...
大多数人认为电能是清洁的。我们的手不会在把插插头时变脏，我们的笔记本电脑也不需要排气管。 然而，由于大部分电能来自于燃烧化石燃料，而能源供应是碳排放[最主要的](https://www.eea.europa.eu/data-and-maps/daviz/change-of-co2-eq-emissions -2#tab-chart_4)因素，我们可以将电能与碳排放直接关联起来。因此，耗电量可以被用来衡量碳排放。
...
- 能量的[国际单位制](https://en.wikipedia.org/wiki/International_System_of_Units)单位是焦耳（J）。
- 功率的单位是瓦特，1 瓦特（W）相当于每秒 1 焦耳的速率。
...
利用率对应计算机资源的使用量，通常以百分比形式给出。以最大容量充分利用的计算机的利用率较高，而没有被完全利用的空闲计算机的利用率较低。
